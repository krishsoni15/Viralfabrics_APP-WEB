...
export default function OrdersClient({
  ...
  const { user, isUser, isSuperAdmin, isMaster, isParty } = useSession();
  ...
  // If user just logged in, clear cached orders and force a fresh fetch
  useEffect(() => {
    ...
    try {
      ...
      if (justLoggedIn === 'true') {
        ...
        // Clear local caches to avoid showing previous user's data
        localStorage.removeItem('orders_cache');
        sessionStorage.removeItem('orders-cache-timestamp');
        // Force refresh and go to page 1
        (async () => {
          try {
            await refreshOrdersWithRetry(2, true);
          } catch (e) {
            console.error('Failed to refresh orders after login:', e);
          }
        })();
      }
    } catch (e) {
      console.warn('Error handling justLoggedIn flag:', e);
    }
  }, [refreshOrdersWithRetry]);
        ...
        {!isParty && showQuickActions && (
          <div className={`p-4 rounded-xl border-2 transition-all duration-300 panel-slide-down ${isDarkMode
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-2">
              ...
              {/* Delete All Orders - Only show when orders exist and user is master */}
              {orders.length > 0 && isMaster && (
                <button
